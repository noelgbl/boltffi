...
// Usage
val handle = sensor.readings { reading ->
    println("Value: \${reading.value}")
}
// Later...
handle.cancel()`}
...
while let batch = sub.popBatch(maxCount: 100) {
  ...
  kotlin={`// Generated
fun readings(): SensorReadingsSubscription
...
<CodeComparison
  title="StreamProducer for multiple subscribers"
